...
Float_t GetAngleFromEnergy(Float_t incomingEnergy, Float_t depositedEnergy);
...
// void writeFile(Long64_t toProcess,Int_t runNum){
  ...
  vector<Float_t> energySortedAM0;
  vector<Float_t> energySortedAM1;
  ...
  tree2->Branch("thetaScat",&thetaScat);
  tree2->Branch("thetaAM0",&thetaAM0);
  tree2->Branch("thetaAM1",&thetaAM1);
  ...
  tree2->Branch("energySortedAM0",&energySortedAM0);
  tree2->Branch("energySortedAM1",&energySortedAM1);
  ...
  // for(int i=0;i<calCoef[channelID].size();i++) eCal+=calCoef[channelID][i]*pow(energy,i);
    ...
    // } else {
    if(time>(EventTime+TimeWindow*1000)){ // New event. Fill tree and clear vectors.
      // treeEvents->Fill();
      // tree2->Fill();

      int indexAM0[hitsAM0];
      int indexAM1[hitsAM1];
      TMath::Sort(hitsAM0,&energyVectorAM0[0],indexAM0);
      TMath::Sort(hitsAM1,&energyVectorAM1[0],indexAM1);
      for(int i=0;i<hitsAM0;i++) energySortedAM0.push_back(energyVectorAM0[indexAM0[i]]);
      for(int i=0;i<hitsAM1;i++) energySortedAM1.push_back(energyVectorAM1[indexAM1[i]]);

      if(energyScat!=0) thetaScat=GetAngleFromEnergy(511,energyScat);
      else thetaScat=0;
      if(energyAM0>0) thetaAM0=GetAngleFromEnergy(energyAM0-energyScat,energySortedAM0[1]);
      // if(energyAM0>0) thetaAM0=GetAngleFromEnergy(energyAM0-energyScat,energySortedAM0[0]);
      else thetaAM0=0;
      if(energyAM1!=0) thetaAM1=GetAngleFromEnergy(energyAM1,energySortedAM1[1]);
      // if(energyAM1!=0) thetaAM1=GetAngleFromEnergy(energyAM1,energySortedAM1[0]);
      else thetaAM1=0;
      ...
      energySortedAM0.clear();
      energySortedAM1.clear();
      amVector.clear();
      channelVector.clear();
      xVector.clear();
      yVector.clear();
...
Float_t GetAngleFromEnergy(const Float_t incomingEnergy, const Float_t depositedEnergy)
{
  ...
  //      << "Scattering angle: " << angle*TMath::RadToDeg() << " degrees" << endl
